...
class QwechatModel extends Model {
     ...
     * @param  $page 分页号
     * @param  $row  每页数量
     * @return 应用列表、应用数量集合 array('list' => $list, 'totalCount' => $totalCount);
     */
    public function applicationManage($page = 1, $row = 20) {
        $map['aid'] = array('eq',session('user_auth.aid'));
        $map['status'] = array('GT', -1);
        $model = M('qwechat');
        $list = $model->where($map)->page($page, $row)->order('id desc')->select();
